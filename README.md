...
デフォルトでは `text/markdown`（純粋な本文のみ）で返却します。メタデータ付きのJSONレスポンスが欲しい場合は `Accept: application/json` を指定してください。クエリパラメータ `?markdown_only=true` を付けるとヘッダーに関係なくMarkdownのみが返ります。
